...
model = TransMIL(n_classes=1).cuda() 
...
for epoch in range(n_epochs):
    ...
    for step, batch in progress_bar:
        ...
        Y_true.extend(label.cpu().detach().numpy().astype(np.uint8))
        Preds.extend(torch.sigmoid(logits).cpu().detach().numpy())
        # Y_true.extend(labels.cpu().detach().numpy().astype(np.uint8))
        # Preds.extend(torch.sigmoid(logits).cpu().detach().numpy())
    ...
    print('Train auc={0}, val acc={1}'.format(auc, acc))
    ...
    if auc > auc_best:
        ...
        },'/raid/eredekop/071024_ST/baselines/checkpoints/092124_TRANSMIL_bleep_norm_sharp_{0}'.format(dataset_type))
