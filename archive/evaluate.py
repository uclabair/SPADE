...
from dataset import PatchDataset_CAM16, PatchDataset_PLCO
...
model = MIL_Attention_fc(size_arg=feature_type).cuda()

state_dict = torch.load('/raid/eredekop/071024_ST/baselines/checkpoints/092124_ABMIL_bleep_norm_PLCO')
...
# np.save('./cache/preds_{0}.npy'.format(dataset), np.array(Preds))
# np.save('./cache/y_true_{0}.npy'.format(dataset), np.array(Y_true))
