...
def train_step(model, loader, optimizer, lr_scheduler, loss_fn, in_dropout = 0.0, print_every = 100, accum_steps = 32, args = None):
    ...
    for batch_id, batch in enumerate(loader):
        ...
        if args.model_type == 'abmil':
            (logits, y_prob, y_hat, a_raw, results_dict) = model(data.float())
        else:
...
def save_attentions(test_loader, args):
    if args.model_type == 'abmil':
        model = MIL_Attention_fc(input_size = args.input_dim, n_classes = args.n_label_bins)
    elif args.model_type == 'transmil':
        model = TransMIL(size_arg = args.feature_type, n_classes=args.n_label_bins)
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(loader):
            ...
            if args.model_type == 'abmil':
                a_raw = model(data.float(), attention_only = True)
            else:
                logits = model(data=data.float())
            

            attentions_all.append(a_raw.detach().cpu().numpy().squeeze())
