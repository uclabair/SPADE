...
'''Momentum class for survival option - modified from torchsurv code'''
class SimpleMomentum(nn.Module):
...
class LinearProbingSimple(nn.Module):
    def __init__(self, feature_dim=512, hidden_dim1=128, hidden_dim2=64, num_classes=1):
        super(LinearProbingModel, self).__init__()
        self.classifier = nn.Sequential(                            # Activation
            nn.Linear(feature_dim, num_classes)    # Output layer
        )
    def forward(self, x):
        return self.classifier(x)