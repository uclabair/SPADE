...
from dataset import PatchDataset_PLCO, PatchDataset_Camelyon, PatchDataset_CPTAC, PatchDataset_PANDA
...
def main(args):
    ...
    if args.task == 'camelyon16':
        ...
        train_dataset = PatchDataset_Camelyon(train_df, mode = 'train', feat_path = args.feat_path)
        val_dataset = PatchDataset_Camelyon(val_df, mode = 'val', feat_path = args.feat_path)
        train_labels = list(train_df['binary'])

    elif args.task == 'cptac_lung':
        train_dataset = PatchDataset_CPTAC(train_df, mode = 'train', feat_path = args.feat_path)
        val_dataset = PatchDataset_CPTAC(val_df, mode = 'val', feat_path = args.feat_path)
        train_labels = list(train_df['label'])
    
    elif args.task == 'panda':
        train_dataset = PatchDataset_PANDA(train_df, mode = 'train', feat_path = args.feat_path)
        val_dataset = PatchDataset_PANDA(val_df, mode = 'val', feat_path = args.feat_path)
